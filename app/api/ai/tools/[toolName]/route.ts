...
/**
 * GET /api/ai/tools/[toolName]
 *
 * Returns the requested tool assignments
 ...
 *   ?agentId=...
 *
 * Without agentId, all assignments for the
 * tool name in the current organization are
 * returned.
 */
export async function GET(
  ...
  try {
    ...
    /**
     * Verify agent ownership when an agentId
     * filter is supplied.
     */
    if (agentId) {
      const agent =
...
/**
 * PATCH /api/ai/tools/[toolName]
 *
 * Updates all matching assignments for the
 * current organization, or a single assignment
...
/**
 * DELETE /api/ai/tools/[toolName]
 *
 * Removes one tool assignment from an agent.
 *
 * Body:
 *
 * {
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "Unable to delete AI tool assignment.",
      },
      {
        status: 500,
      },
